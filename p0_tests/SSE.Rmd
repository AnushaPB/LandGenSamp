...
set.seed(42)
m <- as.matrix(dist(sample(1:10, 10, replace = TRUE), diag = TRUE, upper = TRUE))
diag(m) <- 0
...
m %>%
    ...
    ggplot(aes(x1, x2)) + 
      ...
      scale_fill_gradient2(low = colpal[2], mid = colpal[1], high = colpal[5], midpoint = 5)
...
m %>%
    ...
    ggplot(aes(x1, x2)) + 
      ...
      ggthemes::theme_solid() + 
      theme(legend.position = "none") +
      scale_fill_gradient2(low = colpal[2], mid = colpal[3], high = colpal[4], midpoint = 5)
...
m %>%
    ...
    ggplot(aes(x1, x2)) + 
      ...
      ggthemes::theme_solid() + 
      theme(legend.position = "none") +
      scale_fill_gradient2(low = colpal[5], mid = colpal[2], high = colpal[3], midpoint = 5)
```
