...
boot_rnM <- function(x,
  ...
  for(i in 1:nsamp){
    ...
    pna <- 1
	  ...
	  #calc correlation
	  r <- layerCor(newx, "pearson", na.rm=T)$`pearson`[1,2]
	  
    f <- matrix(c(rep(1,d),0,rep(1,d)), n, n)
    Mt <- autocor(newx[[1]], w = f, method = "moran", global = TRUE)
    Mp <- autocor(newx[[2]], w = f, method = "moran", global = TRUE)
    
	  cor <- append(cor, r)
	  Mtavg <- append(Mtavg, Mt)
	  Mprec <- append(Mprec, Mp)
  }
	return(list(r = cor, Moran_prec = Mprec, Moran_tavg = Mtavg))
}
```
...
```{r}
ggplot(df) +
  geom_boxplot(aes(x = Moran), fill = "cornflowerblue", alpha = 0.5, lwd = 1) +
  theme_classic() +
  ylim(-1,1) +
  theme(axis.text.y = element_blank(),
        axis.line.y = element_blank(),
...
f <- matrix(c(rep(1,d),0,rep(1,d)), n, n)
M_H50 <- autocor(rast(nlm50), w = f, method = "moran", global = TRUE)
M_H05 <- autocor(rast(nlm05), w = f, method = "moran", global = TRUE)
```
