...
adf_test <- function(subdf, nsteps = NULL, var = "mean_fit"){
  # Creating a time series object
  ts <- 
    subdf %>% 
    pull(.data[[var]]) %>%
  ...
  # Note: used na.omit() instead of drop_na(mean_fit) because then the time series would be shorter than the original data
  # and the time points would not line up correctly
  timepoints <- map(seq(0, 9500, nsteps), function(x) x:(x + nsteps))
  p_value <- map_dbl(timepoints, ~tseries::adf.test(na.omit(ts[.x]), alternative = "stationary")$p.value)
...
fitness_df <-
  ...
  map(ls, adf_test, nsteps = 1000, .progress = TRUE) %>% 
  bind_rows() %>%
  mutate(stationarity = p < 0.05) 
...
fitness_addmean <- 
  ...
  # find the first time point that is stationary 
  mutate(first_stationary = min(t_start[stationarity])) %>%
  # fill in everything after that time point with TRUE
  mutate(reached_stationarity = t_start >= first_stationary) %>%
  ungroup() %>%
...
ggplot() +
  geom_rect(aes(xmin = 0, xmax = 1000, ymin = -Inf, ymax = Inf), fill = "lightgray") +
  ...
  scale_fill_viridis_c(option = "mako", end = 1, begin = 0.2, direction = -1, limits = c(5, 100)) +
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), breaks = seq(0, 10000, 2000)) +
  ggtitle("A. Percent of simulations at stationarity in the preceding 1000 timesteps") +
  theme(strip.background = element_blank())
