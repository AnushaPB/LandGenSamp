...
fitness_df <-
  # Test for stationarity in mean fitness
  map(ls, adf_mw_test, nsteps = 2000, .progress = TRUE) %>% 
  bind_rows() %>%
  mutate(stationarity = p < 0.05) 
...
ggplot() +
  #geom_rect(aes(xmin = 0, xmax = 2000, ymin = -Inf, ymax = Inf), fill = "lightgray") +
  geom_rect(data = fitness_mean, aes(xmin = t_start, xmax = t_end, ymin = -Inf, ymax = Inf, fill = stationarity), alpha = 0.9) +
  geom_vline(data = fitness_mean, aes(xintercept = t_end), lty = "dashed", alpha = 0.5, col = "white", lwd = 0.5) +
  ...
  scale_fill_viridis_c(option = "mako", end = 0.8, begin = 0.3) +
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), breaks = seq(0, 10000, 2000)) +
