...
run_lfmm_full <- function(gen, gsd_df, loci_df){
  ...
  FP <- sum(lfmm_loci1 %in% neutral_loci) + sum(lfmm_loci1 %in% loci_trait2)
  FDR1 <- FP/(FP + TP)
  ...
  #calc False Discovery Rate 
  FP <- sum(lfmm_loci2 %in% neutral_loci) + sum(lfmm_loci2 %in% loci_trait1)
  FDR2 <- FP/(FP + TP)
  ...
  #calc True Positive Rate
  TPR1COMBO <- TP1/(TP1 + FN1)
  #calc False Discovery Rate 
  FDR1COMBO <- FP1/(FP1 + TP1)
  ...
  #calc True Positive Rate
  TPR2COMBO <- TP2/(TP2 + FN2)
  #calc False Discovery Rate 
  FDR2COMBO <- FP2/(FP2 + TP2)
  ...
  return(data.frame(K = K,
                    ...
                    TPR1 = TPR1, FDR1 = FDR1, 
                    TPR2 = TPR2, FDR2 = FDR2,
                    TOTALN = length(lfmm_loci), TOTALTP = TP, TOTALFP = FP, TOTALTN = TN,
