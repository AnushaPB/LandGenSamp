...
run_mmrr_params <- function(i, params, path, mode = "ind"){
  ...
  if(skip_to_next == FALSE){
    ...
    result <- data.frame(params[i,], sampstrat = "full", nsamp = nrow(gsd_df_2k), full_result, 
                         ratio = fullratio, env1_err = NA, env2_err = NA, geo_err = NA, ratio_err = NA)
    ...
    sub_result <- map_dfr(sampcombos, run_sub_mmrr, i, params, gen, gsd_df, mode)
...
run_sub_mmrr <- function(sampcombo, i, params, gen, gsd_df, full_result, mode = "ind"){
