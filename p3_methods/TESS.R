...
#  TESS #
#########
run_tess_full <- function(gen, gsd_df, loci_df, paramset){
  #get adaptive loci
  ...
  #create coordinates
  coordinates <- as.matrix(gsd_df[ ,c("x","y")])
  
  #Code for testing multiple K values:
  maxK <- 10
  tess3.obj <- tess3(X = gen, coord = coordinates, K = 1:maxK, method = "projected.ls", ploidy = 2) 
  
  #determining best K and picking best replicate for best K (source: https://chazhyseni.github.io/NALgen/post/determining_bestk/)
  ce <- list()
  for(k in 1:maxK) ce[[k]] <- cross.entropy(tess3.obj, K=k)
  ce.K <- c()
  ...
  #Get Qmatrix
  pred_admix <- qmatrix(tess3.obj, K = K) 
...
run_tess<- function(gen, gsd_df, loci_df, K, full_krig_admix, full_admix){
  #get adaptive loci
  loci_trait1 <- loci_df$trait1 + 1 #add one to convert from python to R indexing
  loci_trait2 <- loci_df$trait2 + 1 #add one to convert from python to R indexing
  adaptive_loci <- c(loci_trait1, loci_trait2)
  ...
  #run TESS
  tess3.obj <- tess3(X = gen, coord = coordinates, K = K, method = "projected.ls", ploidy = 2) 
  ...
  if(skip_to_next == FALSE){
    ...
    for(nsamp in npts){
      for(sampstrat in sampstrats){
        ...
        subresult <- run_TESS(subgen, subgsd_df, loci_df, K = full_result$K, full_krig_admix = full_krig_admix, full_admix = full_admix)
