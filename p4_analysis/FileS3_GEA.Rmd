...
```{r, fig.width = 13, fig.height = 8, include = FALSE}
df <- 
...
ggdf <- 
  ...
  mutate(scenario = case_when(K == 2 & m == 1 & phi == 1 & r == 0.3 & H == 0.5 ~ "best",
                              !(K == 2 & m == 1 & phi == 1 & r == 0.3 & H == 0.5) ~ "worst",
                              .default = NA)) %>%
...
ind_plts <-
  ...
  map(~{
    ...
    geom_line(lwd = 1, aes(col = Scheme), alpha = 0.8) +
    geom_point(cex = 2, aes(col = Scheme, pch = Scheme, fill = Scheme), alpha = 0.8) +
...
site_plts <-
  ...
  map(~{
    ...
    theme(panel.border = element_rect(colour = "lightgray", fill = NA),
          ...
          panel.spacing = unit(0.5, "lines"),
          legend.position = "right") +
    scale_color_manual(values = c( "#0D0887FF", "#17c3b2", "#ED7953FF")) +
    scale_fill_manual(values = c( "#0D0887FF", "#17c3b2", "#ED7953FF")) +
