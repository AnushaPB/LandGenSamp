...
ind_plts <-
  ...
  map(~{
    ...
    geom_line(data = filter(.x, Scheme == "ES"), lwd = 1, alpha = 0.8) +
    geom_point(data = filter(.x, Scheme == "ES"), cex = 2, alpha = 0.8) +
...
df %>%
  ...
  filter(sampling == "individual") %>%
  select(K, phi, m, seed, H, r, it, TPRCOMBO_relaxed, sampstrat, Method, nsamp) %>%
  pivot_wider(names_from = sampstrat, values_from = TPRCOMBO_relaxed) %>%
  group_by(K, phi, m, seed, H, r, it, nsamp, Method) %>%
  mutate(T_dif = mean(c(T - ES, T - G, T - R))) %>%
  group_by(Method) %>%
  summarize(round(mean(T_dif),3))
...
df %>%
  ...
  summarize(dif = round(mean(dif),3)) %>%
  arrange(-dif)
