...
```{r}
stats <- c("TPRCOMBO_strict", "FDRCOMBO_strict")
```

## LFMM
### Individual sampling
```{r, fig.width = 25, fig.height = 15}
map(stats, ~megalineplot(lfmm_ind, stat = .x, sampling = "ind"))
```

### Site sampling
```{r, fig.width = 25, fig.height = 15}
map(stats, ~megalineplot(lfmm_site, stat = .x, sampling = "site"))
```

## RDA
### Individual sampling
```{r, fig.width = 25, fig.height = 15}
map(stats, ~megalineplot(rda_ind, stat = .x, sampling = "ind"))
```

### Site sampling
```{r, fig.width = 25, fig.height = 15}
map(stats, ~megalineplot(rda_site, stat = .x, sampling = "site"))
```
...
# Under the best conditions with 25 sites using RDA, E-space sampling had a true positive rate of XXX compared to YYY for random sampling and ZZZ for grid sampling
df %>%
  ungroup() %>%
  filter(K == 2, phi == 1, m == 1, H == 0.5, r == 0.3) %>%
