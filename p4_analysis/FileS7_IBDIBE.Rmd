...
Average increase in IBD error under high migration for individual-based sampling:
```{r, include = TRUE, results=TRUE}
df %>%
  ungroup() %>%
  filter(sampling == "individual") %>%
  select(K, phi, m, seed, H, r, it, geo_coeff_err_ae_scale, sampstrat, Method, nsamp) %>%
  pivot_wider(names_from = m, values_from = geo_coeff_err_ae_scale) %>%
  group_by(K, phi, seed, H, r, it, nsamp, Method) %>%
  mutate(dif = mean(`1` - `0.25`)) %>%
  group_by(Method) %>%
  summarize(
    `Mean difference` = round(mean(dif, na.rm = TRUE),3), 
    `SD difference` = round(mean(dif, na.rm = TRUE),3)
    ) 
```
...
df %>%
  ...
  summarize(
    ...
    `SD difference` = round(mean(T_dif, na.rm = TRUE),3)
    ) 
```
