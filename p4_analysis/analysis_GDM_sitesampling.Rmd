...
df <- read.csv(here("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/gdm_sitesampling_results.csv"))
...
df$geo_err <- abs(as.numeric(df$geo_err))
df$env1_err <- abs(as.numeric(df$env1_err))
df$env2_err <- abs(as.numeric(df$env2_err))
...
df$ratio <- (df$geo_coeff - df$comboenv_coeff)/df$comboenv_coeff

df$comboenv_err <- (df$env1_err + df$env2_err)/2

# TEST REMOVE
df$env1_coeff[df$env1_coeff == 0] <- NA
df$env2_coeff[df$env2_coeff == 0] <- NA
df$geo_coeff[df$geo_coeff == 0] <- NA



df$comboenv_coeff <- df$env1_coeff + df$env2_coeff
df$ratio <- df$geo_coeff/df$comboenv_coeff
...
for(i in 1:nrow(params)){
  ...
  full$ratio_err <- NA
  full$ratio_mpe <- NA
  sub$ratio_err <- full$ratio -  sub$ratio
  sub$ratio_mpe <- abs(full$ratio -  sub$ratio)/abs(full$ratio)
  cdf <- rbind(cdf, sub, full)
}

summary_hplot(cdf, "ratio_err")
summary_hplot(cdf, "ratio_mpe")

summary_hplot(cdf, "ratio")
summary_vplot(cdf, "ratio")

summary_hplot(cdf, "geo_coeff")
summary_hplot(cdf, "env1_coeff")
summary_hplot(cdf, "env2_coeff")
```
```{r fig.width = 30, fig.height=15}

MEGAPLOT(cdf, "ratio_err", option = "turbo")
```
...
MEGAPLOT(moddf, "comboenv_err", aggfunc = "prop_na", option = "turbo")
MEGAPLOT(moddf, "geo_err", aggfunc = "prop_na", option = "turbo")
MEGAPLOT(moddf, "geo_coeff", aggfunc = "prop_na", option = "turbo")

MEGAPLOT(moddf, "env1_coeff", aggfunc = "prop_na", option = "turbo")
MEGAPLOT(moddf, "env2_coeff", aggfunc = "prop_na", option = "turbo")
```
...
summary_hplot(df, "comboenv_coeff", aggfunc = "prop_na", colpal = "viridis", direction = -1)
```

```{r, fig.width = 30, fig.height=15}
MEGAPLOT(df, "geo_coeff")
MEGAPLOT(df, "ratio", option = "turbo")
```
