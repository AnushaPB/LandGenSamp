...
df <- format_data("mmrr", "individual") 
...
plot_stats <- c("TPRCOMBO_relaxed", "FDRCOMBO_relaxed")
results <- list(lfmm_ind, lfmm_site, rda_ind, rda_site)

filt_results <- map(results,  ~filter(.x, H == 0.5, m == 1, K == 2, phi == 1, r == 0.3))
...
sub_results <- map(filt_results, \(x){
  ...
  x <- 
     ...
     summarize_at(plot_stats, mean, na.rm = TRUE, .groups = "keep") 
...
plots <-
  map(sub_results, \(df) map(map_chr(plot_stats, make_pretty_names), ~heat_plot(
    df,
    stat_name = .x,
...
pdf(here("plots", "GEA_ind.pdf"), width = 6, height = 6)
do.call(ggarrange, c(plot_ind, ncol = 1, align = "h"))
dev.off()

pdf(here("plots", "GEA_site.pdf"), width = 6, height = 6)
do.call(ggarrange, c(plot_site, ncol = 1, align = "h"))
dev.off()

pdf(here("plots", "GEA_legends.pdf"), width = 6, height = 6)
