...
df <-
  df %>%
  filter(sampstrat == "R", H == 0.5, phi == 1.0, r == 0.3, m == 1.0, K == 2.0)

aggdf <- 
  df %>%
  group_by(nsamp, method, sampling) %>%
  summarize(TPRCOMBO_relaxed = mean(TPRCOMBO_relaxed, na.rm = TRUE), 
            FDRCOMBO_relaxed = mean(FDRCOMBO_relaxed, na.rm = TRUE)) %>%
  pivot_longer(c(TPRCOMBO_relaxed, FDRCOMBO_relaxed)) %>%
...
ind_plt <- sampling_plot(ggdf[ggdf$sampling == "individual",], aggdf[aggdf$sampling == "individual",])
site_plt <- sampling_plot(ggdf[ggdf$sampling == "site",], aggdf[aggdf$sampling == "site",])
