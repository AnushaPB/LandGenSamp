...
ibdibe_figplot <- function(df){
  df_stats <- df %>% ungroup() %>% dplyr::select(-nsamp, -sampstrat) %>% colnames()
  pretty_stats <- map_chr(df_stats, make_pretty_names)
  result <-
    map(pretty_stats,
        ~ heat_plot(
          df,
          stat_name = .x,
          maxv = ifelse((grepl("TPR", .x) | grepl("FDR", .x)), 1.01, 0.4),
          minv = ifelse((grepl("TPR", .x) | grepl("FDR", .x)), 0,-0.3)
        ))
  return(result)
}
...
sub_results <- map(IBDIBE_stats, \(stats) map(filt_results, ~summarize_stats(.x, stats)))

plots <- map(sub_results, ~transpose(map(.x, ibdibe_figplot)))

plot_ind <- map(plots, \(plot) map(plot, ~ggarrange(.x[[1]], .x[[3]], common.legend = TRUE, nrow = 1, legend = "none")))

plot_site <- map(plots, ~ggarrange(.x[[2]], .x[[4]], common.legend = TRUE, nrow = 1, legend = "none"))

pdf(here("plots", "IBDIBE_ind.pdf"), width = 6, height = 12)
do.call(ggarrange, c(plot_ind, ncol = 1, align = "h"))
dev.off()

pdf(here("plots", "IBDIBE_site.pdf"), width = 6, height = 12)
do.call(ggarrange, c(plot_site, ncol = 1, align = "h"))
dev.off()

pdf(here("plots", "IBDIBE_legends.pdf"), width = 6, height = 12)
