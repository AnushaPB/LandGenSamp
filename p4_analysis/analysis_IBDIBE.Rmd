...
```{r, fig.width=14, fig.height=7}
gdm_ind <- format_gdm(here(p3path, "gdm2_indsampling_results_test2.csv"))
...
df <-
  df %>%
  filter(sampstrat == "R") %>%
  mutate(simulation = case_when((H == 0.5 & phi == 1.0 & r == 0.3 & m == 0.25 & K == 1.0) ~ "best conditions",
                        (H != 0.5 & phi != 1.0 & r != 0.3 & m != 0.25 & K != 1.0) ~ "worst conditions"))

ggarrange(ibdibe_nsamp_plot(df, sampling = "individual", stats = stats) + 
            ggtitle("A. Individual-based sampling"), 
          ibdibe_nsamp_plot(df, sampling = "site", stats = stats) +
...
a <- read.csv(here("p3_methods/outputs/mmrr2_indsampling_results.csv"))
b <- read.csv(here("p3_methods/outputs/gdm2_indsampling_results_test2.csv"))
