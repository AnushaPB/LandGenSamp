...
filt_results <- map(results,  ~filter(.x, H != 0.05, m != 1.00, K == 2))
...
sub_results <- map(filt_results, \(x){
  ...
  x <- 
     ...
     summarize(`IBE TPR` = mean(env_p_TPR, na.rm = TRUE), 
               ...
               `IBE AE` = mean(env_coeff_err_ae, na.rm = TRUE),
               `IBD AE` = mean(geo_coeff_err_ae, na.rm = TRUE),
...
IBETPR_plots <- map(sub_results, ~heat_plot(.x, stat_name = "IBE TPR", maxv = 1, minv = 0.1, 
                 colpal = "plasma", direction = 1))

IBDTPR_plots <- map(sub_results, ~heat_plot(.x, stat_name = "IBD TPR", maxv = 1, minv = 0.1, 
                 colpal = "plasma", direction = 1))
