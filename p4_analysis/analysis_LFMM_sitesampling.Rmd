...
#df <- read.csv("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/LFMM_sitesampling_results_NOAGGREGATION.csv")
df <- read.csv("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/LFMM_sitesampling_results_tw.csv")
...
MEGAPLOT(moddf, "TPRCOMBO", maxv = 0.9, option = "turbo")
...
#EXPLORATORY
```{r, fig.width=20, fig.height=7}
df$diff <- 1-df$TPRCOMBO/1-df$FDRCOMBO

summary_hplot(df, "diff")
```

```{r fig.width=20, fig.height=7}
summary_hplot(df, "TNRCOMBO")
summary_hplot(df, "FDRCOMBO")

summary_hplot(df, "TPRCOMBO", aggfunc = "var")

summary_vplot(df, "TPRCOMBO")
summary_hplot(df, "K.1")

```

```{r fig.width=10, fig.height=10}
site_hist <- function(df, stat, breaks = seq(0,1,0.10), xlim, ylim){
    colv <- list(c(0,1,1),c(1,0,1),c(0,1,0))
    par(mfrow=c(3,3))
    for(n in nsamp[-4]){
        c <- 1
        for(s in sampstrat[-4]){
...
site_hist(df, "TOTALN", breaks = seq(0, max(df$TOTALN)+20, 20), ylim = c(0, 50), xlim = range(df$TOTALN))
site_hist(df, "TOTALT", breaks = seq(0, 9, 1), ylim = c(0, 80), xlim = c(0,9))
site_hist(df, "TOTALF", breaks = seq(0, max(df$TOTALN)+20, 20), ylim = c(0, 80), xlim = range(df$TOTALN))
```
...
```{r fig.width = 10, fig.height = 10}
site_auc <- function(df){
    colv <- list(c(0,1,1),c(1,0,1),c(0,1,0))
    par(mfrow=c(3,3))
    for(n in nsamp[-4]){
        c <- 1
        for(s in sampstrat[-4]){
            d <- df[df$nsamp == n & df$sampstrat == s,]
            plot(x= 1- d$FPRCOMBO,
                 y= d$TPRCOMBO,
                 main = paste0(n, " | ", s ," | ", (mean(sample(d$TOTALTP,1000,replace=T) > sample(d$TOTALTN,1000,replace=T)))), 
                 xlab = "FPR", ylab = "TPR",
                 col = rgb(colv[[c]][1],colv[[c]][2],colv[[c]][3], alpha = 0.5),
                 pch = 19)
        }
    }
...
colv <- list(c(0,1,1),c(1,0,1),c(1,1,0))
par(mfrow=c(1,3))
for(n in nsamp[-4]){
    c <- 1
    plot(1, type="n", xlab="", ylab="", xlim=c(0, 1), ylim=c(0, 70))
    for(s in sampstrat[-4]){
        d <- df[df$nsamp == n & df$sampstrat == s, "TPRCOMBO"]
        hist(d, main = "", xlim = c(0,1), xlab = "", ylab = "", add = T, 
             col = rgb(colv[[c]][1],colv[[c]][2],colv[[c]][3], alpha = 0.2))
        c <- c+1
    }
}
...
for(s in sampstrat[-4]){
    ...
    for(n in nsamp[-4]){
        d <- df[df$nsamp == n & df$sampstrat == s, "TPRCOMBO"]
        hist(d, main = "", xlim = c(0,1), xlab = "", ylab = "", add = T, 
             col = rgb(colv[[c]][1],colv[[c]][2],colv[[c]][3], alpha = 0.1))
        c <- c+1
    }
}
```
