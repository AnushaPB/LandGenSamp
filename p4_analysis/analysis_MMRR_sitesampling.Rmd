...
df <- do.call(rbind, lapply(list.files(path = "/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/sitesampling/outputs/MMRR/", full.names = TRUE, pattern="*.csv"), read.csv))
...
for(i in 1:nrow(params)){
  ...
  fullratio <- (full$geo_coeff - (full$env1_coeff + full$env2_coeff))/full$geo_coeff
  subratio <- (sub$geo_coeff - (sub$env1_coeff + sub$env2_coeff))/sub$geo_coeff
  sub$ratio2_ae <- abs(fullratio - subratio)
  cdf <- rbind(cdf, sub)
}
```
...
for(i in 1:nrow(params)){
  ...
  full <- subdf[subdf$sampstrat == "full",]
  sub <- subdf[subdf$sampstrat != "full",]
  sub$err <-  abs(sub$geo_coeff - full$geo_coeff)
  cdf <- rbind(cdf, sub)
}

summary_hplot(cdf, "err")
```


#EXPLORATORY

<<<<<<< Updated upstream:p4_analysis/sitesampling/analysis_MMRR_sitesampling.Rmd
...
MEGAPLOT(moddf, "geo_coeff", option = "turbo", na.rm = TRUE)

MEGAPLOT(moddf, "geo_rmse", option = "turbo", na.rm = TRUE)
MEGAPLOT(moddf, "comboenv_rmse", option = "turbo", na.rm = TRUE)
>>>>>>> Stashed changes:p4_analysis/analysis_MMRR_sitesampling.Rmd
```

```{r fig.width=20, fig.height=8}
#should be close to 1 since env1 and env2 should have equal impact (not sure if this is true though because it could vary based on overlap between env1 and 2)
df$stat <- abs(1 - abs(df$env1_coeff/df$env2_coeff))
summary_hplot(df)


cdf$stat <- cdf$ratio_rmse
summary_hplot(cdf, colpal = "viridis", sigdig = 3)

cdf$stat <- cdf$ratio2_ae
summary_hplot(cdf, colpal = "viridis", sigdig = 3)
summary_hplot(cdf, colpal = "viridis", sigdig = 3, aggfunc = "var")

cdf$stat <- log(cdf$ratio2_ae)
summary_hplot(cdf, colpal = "viridis", sigdig = 3)
summary_hplot(cdf, colpal = "viridis", sigdig = 3, aggfunc = "var")
```
