...
fullmod <- lmer(stat ~ npts + sampstrats + K + m + phi + H + r + (1 | seed), df, na.action = "na.fail")
...
ggplot(stats_out[-1,], aes(factor(nsamp), sampstrat)) +
    geom_tile(aes(fill = env1_rmse)) + 
    geom_text(aes(label = signif(env1_rmse,  digits = 2), hjust = 0.5)) +
    scale_fill_gradient2(low = "#2066AC", mid="#F7F7F7", high = "#B2182B", midpoint = 0, limits=c(min(stats_out$env1_rmse),max(stats_out$env1_rmse))) +
    theme_bw() +
    theme(panel.border = element_blank(), panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(), 
          #legend.position = "none",
...
ggplot(stats_out[-1,], aes(factor(nsamp), sampstrat)) +
    geom_tile(aes(fill = env2_rmse)) + 
    geom_text(aes(label = signif(env2_rmse,  digits = 2), hjust = 0.5)) +
    scale_fill_gradient2(low = "#2066AC", mid="#F7F7F7", high = "#B2182B", midpoint = 0, limits=c(min(stats_out$env2_rmse),max(stats_out$env2_rmse))) +
    theme_bw() +
    theme(panel.border = element_blank(), panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(), 
          #legend.position = "none",
...
ggplot(stats_out[-1,], aes(factor(nsamp), sampstrat)) +
    geom_tile(aes(fill = geo_rmse)) + 
    geom_text(aes(label = signif(geo_rmse,  digits = 2), hjust = 0.5)) +
    scale_fill_gradient2(low = "#2066AC", mid="#F7F7F7", high = "#B2182B", midpoint = 0, limits=c(min(stats_out$geo_rmse),max(stats_out$geo_rmse))) +
    theme_bw() +
    theme(panel.border = element_blank(), panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(), 
          #legend.position = "none",
