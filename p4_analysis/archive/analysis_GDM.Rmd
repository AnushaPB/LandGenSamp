...
df <- read.csv("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/gdm_indsampling_results.csv")
...
```
```{r}
unique(df[is.na(df$geo_coeff), 1:9])
```
...
for(i in 1:nrow(params)){
  ...
  full$absratio <- (abs(full$env1_coeff) + abs(full$env2_coeff))/abs(full$geo_coeff)
  sub$absratio <- (abs(sub$env1_coeff) + abs(sub$env2_coeff))/abs(sub$geo_coeff)
  
  sub$ratio_ae <- abs(sub$absratio - full$absratio)
  sub$ratio_e <- sub$absratio - full$absratio
  sub$ratio_te <- atan(abs(sub$absratio - full$absratio)/abs(full$absratio))
  sub$ratio_re <- abs(sub$absratio - full$absratio)/(abs(full$absratio) + 1)
  
  sub$comboenv_ae <- abs(sub$comboenv_coeff - full$comboenv_coeff)
  sub$geo_ae <- abs(sub$geo_coeff - full$geo_coeff)
...
summary_hplot(cdf, "ratio_ae", colpal = "viridis", direction = -1)
summary_hplot(cdf, "ratio_re", colpal = "viridis", direction = -1)
summary_hplot(cdf, "geo_e", colpal = "viridis", direction = -1)
summary_hplot(cdf, "comboenv_e", colpal = "viridis", direction = -1)

```
#summary plots

```{r fig.width = 30, fig.height=15}
MEGAPLOT(cdf, "ratio_e")
MEGAPLOT(cdf, "geo_e")
MEGAPLOT(cdf, "comboenv_e")
```

```{r fig.width = 30, fig.height=15}
MEGAPLOT(cdf, "geo_mpe", option = "turbo", max = 0.6, na.rm=TRUE)
MEGAPLOT(cdf, "comboenv_mpe", option = "turbo", max = 3, na.rm=TRUE)
```
```{r fig.width = 30, fig.height=15}
MEGAPLOT(moddf, "ratio", option = "turbo", maxv = 0.3,  na.rm=TRUE)
```
```{r fig.width = 30, fig.height=15}
MEGAPLOT(moddf, "comboenv_err", aggfunc = "prop_na", option = "turbo", maxv = 0.14,  na.rm=TRUE)
MEGAPLOT(moddf, "geo_err", aggfunc = "prop_na", option = "turbo", maxv = 0.14,  na.rm=TRUE)

MEGAPLOT(moddf, "geo_coeff", aggfunc = "prop_na", option = "turbo", maxv = 0.14,  na.rm=TRUE)
MEGAPLOT(moddf, "env1_coeff", aggfunc = "prop_na", option = "turbo", maxv = 0.14,  na.rm=TRUE)
MEGAPLOT(moddf, "env2_coeff", aggfunc = "prop_na", option = "turbo", maxv = 0.14,  na.rm=TRUE)
```

```{r fig.width = 30, fig.height=15}
MEGAPLOT(moddf, "geo_err", option = "turbo", max = 0.8, na.rm=TRUE)
MEGAPLOT(moddf, "comboenv_err", option = "turbo", max = 0.20, na.rm=TRUE)
```
