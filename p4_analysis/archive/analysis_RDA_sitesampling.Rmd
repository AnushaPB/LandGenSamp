...
#df <- read.csv("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/rda_sitesampling_results_NOAGGREGATION.csv")
df <- read.csv("/Users/Anusha/Documents/GitHub/LandGenSamp/p3_methods/outputs/rda_sitesampling_results.csv")
...
moddf <- moddf[moddf$padj == "fdr" & moddf$alpha == 0.05, ]
...
summary_hplot(moddf, "TPR")
summary_hplot(moddf, "FDR")

summary_hplot(moddf, "TPR", aggfunc = "var")

summary_vplot(moddf, "TPR")
...
site_hist(df, "TPR", breaks = seq(0,1,0.125), ylim = c(0, 80), xlim = c(0,1))
site_hist(df, "FDR", breaks = seq(0,1,0.05), ylim = c(0, 100), xlim = c(0,1))
...
for(n in nsamp[-4]){
    ...
    for(s in sampstrat[-4]){
        d <- df[df$nsamp == n & df$sampstrat == s, "TPR"]
